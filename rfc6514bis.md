...
abbrev: BGP Encodings and Procedures for MVPNs
docname: draft-ietf-bess-rfc6514bis-00
category: std
...
--- abstract
   ...
   This document updates and obsoletes RFC 6514. The original authors of
   RFC 6514 are listed at the end of this document.
...
#Introduction
   ...
   exchanging the information elements required by Multicast in MPLS/BGP
   IP VPNs, as specified in {{!RFC6513}}.  This document assumes a thorough
...
#PMSI Tunnel Attribute
   ...
   * Leaf Information Required (L)
   ...
   If the Label field is non-zero, then it contains a de-multiplex value
   that identifies the VPN. It MAY be one of the following:

   * An MPLS label encoded as 3 octets, where the high-order 20 bits
     contain the label
     value.  Absence of an MPLS Label is indicated by setting the MPLS
     Label field to zero.
   * An VNI/VSID value if VXLAN/NVGRE is used for the provider tunnel
     encapsulation {{?RFC8365}} {{?I-D.zzhang-bess-mcast-in-evpn-signaled-l3vpn}}.
   * The whole or a portion of the Function part of the SRv6 Multicast Service
     SID {{?I-D.ietf-bess-mvpn-evpn-sr-p2mp}}.
   ...
   Equivalence Class (FEC) Element {{?RFC6388}}.
   ...
   LSP, the Tunnel Identifier is an MP2MP FEC Element {{?RFC6388}}.
...
#VRF Route Import Extended Community
   ...
   * The Local Administrator field of the C-multicast Import RT
   ...
   If a PE uses Route Target Constraint {{?RFC4684}}, the PE SHOULD
...
##MVPN Auto-Discovery/Binding - Intra-AS Operations
...
###Originating Intra-AS I-PMSI A-D Routes {#nonsegpmsi}
   ...
   The route has an MCAST-VPN NLRI with the RD set to the RD
...
{::comment}
   In the case of Global Table Multicast (GTM) with BGP-MVPN {{!RFC7716}},
   while {{!RFC7716}} specifies that RD 0:0 is used, this document updates
   {{!RFC7716}} that the PEs MUST use unique non-0:0 RDs for their Intra-AS
   I-PMSI A-D routes in the following situation ({{nonseg}}):

   * The provider network is IPv6, and,
   * Non-segmented Inter-AS tunnels are used, and,
   * Route Target Constraint {{?RFC4684}} is not used

   In this situation, the OPTIONAL use of non-upstream-node-identifying RTs
   in {{!RFC7716}} becomes MANDATORY.
{:/comment}
   ...
   can be found in {{!RFC6513}}).  Depending on the technology used for the
...
###Receiving Intra-AS I-PMSI A-D Routes
   ...
   {{!RFC6513}}, such a route will be ineligible for the UMH selection.  This
...
##MVPN Auto-Discovery/Binding - Inter-AS Operations {#interas}
   ...
   * An ASBR MUST be configured with a set of (import) Route Targets
   ...
   (RTs) by using Route Target Constraint {{?RFC4684}}.
   ...
   * The ASBR MUST be (auto-)configured with an import Route Target
   ...
   routes (see Section 11).
      ...
      If the ASBR supports Route Target Constraint {{?RFC4684}}, the
...
###Originating Inter-AS I-PMSI A-D Routes
...
####Inter-AS I-PMSI A-D Route Received via EBGP
...
#####Originating Leaf A-D Route into EBGP
...
####Leaf A-D Route Received via EBGP
...
####Inter-AS I-PMSI A-D Route Received via IBGP
...
####Leaf A-D Route Received via IBGP
...
####Optimizing Bandwidth by IP Filtering on ASBRs
...
#Non-Congruent Unicast and Multicast Connectivity
   ...
   If an MVPN site is multihomed to several PEs, on each of these PEs,
...
The use and the meaning of these fields are as follows:
   ...
   * Length: the length, in bits, of the address prefix.
   * Prefix: contains a Route Distinguisher as defined in
...
#Exchange of C-Multicast Routing Information among PEs
...
##Originating C-Multicast Routes by a PE
...
###Originating Routes: PIM as the C-Multicast Protocol
...
####Originating Source Tree Join C-Multicast Route
...
####Originating Shared Tree Join C-Multicast Route

   Whenever (a) a C-PIM instance on a particular PE creates a new
   (C-\*,C-G) state, and (b) the selected upstream PE for the C-RP
   corresponding to the C-G (see {{!RFC6513}}) is not the local PE, then the
...
###Originating Routes: mLDP as the C-Multicast Protocol
...
###Constructing the Rest of the C-Multicast Route
   ...
   The local PE executes the procedures of {{!RFC6513}} to find the selected
   ...
   the upstream PE to reach that address are as specified in {{!RFC6513}}.
...
#### Intra-AS Scenario
   ...
   The C-multicast route is then advertised only into IBGP. The propagation
   of the route MAY be controlled by the Route Target Constraint {{?RFC4684}}.

#### Inter-AS Scenario

   This is when the local and the upstream PEs are in different ASes.

   If the deployment uses the Route Target Constraint {{?RFC4684}}
   ...
   the advertised MCAST-VPN NLRI is set to the RD of the VPN-IP route
   that contains the address carried in the Multicast Source field,
   and the route is advertised subject to the Route Target Constraint.

   Otherwise, the route is propagated along the reverse path of the
   propagation of the Inter-AS I-PMSI route for the source AS
   in the case of segmented Inter-AS tunnels ({{reversepath}}) or the
   Intra-AS I-PMSI route from the upstream PE in the case of non-segmented
   Inter-AS tunnels. This is achieved as follows.

##### Segmented Inter-AS Tunnels

   The local PE finds in its VRF an Inter-AS I-PMSI A-D route whose Source
...
##### Non-segmented Inter-AS Tunnels {#nonseg}
   ...
   everywhere, their reverse paths are utilized as follows.
   ...
   the found Intra-AS I-PMSI A-D route if it is IPv4, or 0 otherwise.

{::comment}
   Note that the above requires that each PE uses a unique RD for its
   Intra-AS I-PMSI A-D routes. {{!RFC7716}} specifies that in the case
   of Global Table Multicast (GTM) with BGP-MVPN, RD 0:0 is used. This
   document updates {{?RFC7716}} that if non-segmented Inter-AS tunnels
   are used for GTM in the case of an IPv6 provider network,
   the Intra-AS I-PMSI A-D routes MUST have unique non-0:0 RDs
   ({{nonsegpmsi}}), therefor the C-Multicast routes will have corresponding
   non-0:0 RDs.
{:/comment}
   
   The following apply in both the segmented and non-segmented Inter-AS
   tunnels case.
...
##Propagating C-Multicast Routes by an ASBR

   This section applies only if the Route Target Constraint mechanism is
   not used.
   ...
   against the RD of the Intra-AS I-PMSI A-D routes.
...
##Receiving C-Multicast Routes by a PE
...
###Receiving Routes: PIM as the C-Multicast Protocol
...
####Receiving Source Tree Join C-Multicast Route
   ...
   procedures described in Section 9.1 of {{!RFC6513}}.

####Receiving Shared Tree Join C-Multicast Route
...
###Receiving Routes: mLDP as the C-Multicast Protocol
...
##C-Multicast Routes Aggregation
...
##Originating S-PMSI A-D Routes
   ...
   * If a PE originates S-PMSI A-D routes with the Leaf Information
   ...
   route are described in Section 12.3.)
      ...
      xTarget MUST be set to 0.
   
      If the PE supports Route Target Constraint {{?RFC4684}}, the PE
...
##Handling S-PMSI A-D Routes by ASBRs
...
##Receiving S-PMSI A-D Routes by PEs
...
#Switching from Shared a C-Tree to a Source C-Tree
   ...
   described in {{!RFC6513}}, may not suffice.  To illustrate this, consider a
   set of PEs {PE2, PE4, PE6} that are on the C-RP tree for (C-\*,C-G)
   and have chosen a consistent upstream PE, as described in {{!RFC6513}}, for
...
##Source within a Site - Source Active Advertisement
...
##Receiving Source Active A-D Route
...
#Supporting PIM-SM without Inter-Site Shared C-Trees
...
##Discovering Active Multicast Sources {#discovering-active-sources}
   ...
   use PIM Anycast RP procedures {{?RFC4610}} to convey information
   about active multicast sources from one or more of the MVPN C-RPs to
   the PE.  Yet another way is to use MSDP {{?RFC3618}} to convey information
...
#Scalability Considerations
   ...
   A PE should use Route Target Constraint {{?RFC4684}} to advertise
...
###Dampening Withdrawals of C-Multicast Routes
...
###Dampening Source/Shared Tree Join C-Multicast Routes
...
##Dampening Withdrawals of Leaf A-D Routes
...
#Security Considerations
   ...
   discussed in {{?RFC4111}}.  {{!RFC6513}} discusses additional threats specific
...
This document is a bis-update to RFC6514, whose original authors are listed
below.
...
The authors thanks reviews, comments and suggestions from Vinod Kumar Nagaraj.